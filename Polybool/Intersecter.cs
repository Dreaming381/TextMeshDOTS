			...
			while (eventQueue.Length > 0)
			{
				...
				if (ev.isStart)
				{
					...
					if (eve == EventBool.Empty && hasBelow)
						eve = CheckIntersection(ev, below); //expensive: ~30% of function execution time, (all which in DivideEvent -> eventQueue.IndexOf(ev.other)))
					...
					if (eve != EventBool.Empty)
					{
						...
						eventQueue.RemoveAt(eventQueue.Length - 1);
				...
				else
				{
					...
					if (evOtherInStatus > 0 && evOtherInStatus < statusQueue.Length - 1)
						CheckIntersection(statusQueue[evOtherInStatus - 1], statusQueue[evOtherInStatus + 1]);
